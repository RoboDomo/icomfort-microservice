...
const LCCID = process.env.LENNOX_LCCID,
  ...
  MODES_SCHEDULES = `${LENNOX_HOST}/ModesSchedules`,
...
class iComfort extends EventEmitter {
  ...
  async setMode(mode) {
    ...
    debug(
      ...
      const uri = `${MODES_SCHEDULES}/ChangeSystemModeManual`;
      const res = await this.request
        .post(uri)
        .timeout(TIMEOUT)
        .field("zoneId", this.zone)
        .field("scheduleId", 19)
        .field("lccId", `${LCCID}`)
        .field("periodId", 0)
        .field("mode", mode)
      ...
      debug(this.host, `setMode exception `, e);
      this.emit("exception", e);
      return false;
    }
  }
  ...
  async setTargetTemperature(coolTarget, heatTarget) {
    ...
    const uri = `${DASHBOARD}/ChangeSetPoint?zoneId=${
      this.zone
    }&lccId=${LCCID}&coolSetPoint=${coolTarget}&heatSetPoint=${heatTarget}&isPerfectTempOn=false&_=${Date.now()}`;
    try {
      const res = await this.request
        .get(uri)
        .timeout(TIMEOUT)
